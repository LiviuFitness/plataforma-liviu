...
export default async function LayoutCliente({
  ...
  const [{ count: mensajesSinLeer }, { count: revisionesSinLeer }] = await Promise.all([
    supabase
      .from("mensajes")
    ...
    supabase
      ...
      .select("id", { count: "exact", head: true })
      .eq("cliente_id", user.id)
      .gt("creado_en", perfil?.revisiones_visto_en ?? "1970-01-01"),
  ]);

  const hayChatSinLeer = (mensajesSinLeer ?? 0) > 0;
  const hayRevisionSinLeer = (revisionesSinLeer ?? 0) > 0;
