...
export default async function PaginaSesionPresencial({
  ...
  for (const sesion of (previas ?? []) as unknown as FilaSesionPrevia[]) {
    ...
    for (const [ejercicioId, series] of porEjercicio) {
      if ((historial.get(ejercicioId)?.length ?? 0) >= 5) continue;
      ...
      if (!anterior.has(ejercicioId)) anterior.set(ejercicioId, items);
      const kgs = series.map((s) => s.kg).filter((k): k is number => k !== null).map(Number);
      historial.set(ejercicioId, [
        ...(historial.get(ejercicioId) ?? []),
        {
